...
- `claude/pitfall/`：各技术栈踩坑记录数据目录，由 pitfall skill 写入，进版本控制- `claude/settings.json`：Claude Code 全局 settings 的版本化快照（permissions、enabledPlugins、effortLevel、theme 等通用可移植配置），进版本控制；与本机 `~/.claude/settings.json` 通过 cp 双向同步，不做 symlink（settings.json 会被 CLI 动态写入，symlink 易被原子写替换而漂移）。机器私有项放本机 `~/.claude/settings.local.json`，不进本仓
