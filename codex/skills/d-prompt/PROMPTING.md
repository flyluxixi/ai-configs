...
用户需求
  → ① 适用判断（UI 需求 还是 通用图）
  → ①.5（仅 UI）继承项目视觉锚点：查 docs/design/design-system.md
       ├─ 有 → 继承北极星产品 + tokens，风格锚点已满足，跳过风格追问
       └─ 无 → 临时选一个 + 提示用户用 d-decision 固化
  → ② 需求评估：关键维度齐不齐？给没给参考图？
       ├─ 齐 / 有参考 / 已继承锚点 → 直接优化
...
1. **查项目视觉锚点**：约定文件 `docs/design/design-system.md`（项目 git 根下），或用户当场给的北极星产品 / 品牌色。
...
3. **无 → 临时选一个 + 推回正确的层**：用产品锚点库选合理默认生成本次概念图，但**必须明确告诉用户**：
   ...
   > 本次只是临时视觉锚点，仅保证这一张概念图的风格。要让前端不变形、能上线，请用 `d-decision` 把「北极星产品 + 6~7 个核心 token」固化到 `docs/design/design-system.md`，并落进组件库主题（如 shadcn）。否则每次生图、每个页面都会各走各的。

   不要让 d-prompt 自己「定义」视觉语言——那是把设计系统放错层（存进扔掉的概念图里，而非生效的代码里）。

> `design-system.md` 建议结构（供用户固化时参考）：北极星产品 / 主色 / 中性色阶 / 语义色（成功 warning danger）/ 圆角档位 / 间距密度 / 字体族与层级 / 阴影层级 / 图标与插画风格。
...
4. **推断项清单**（逐条列出你替用户补的风格/配色/组件/构图等，可否决；走参考图驱动时附从参考图提取的风格特征）。UI 路径若**继承了项目视觉锚点**，注明「已继承 `docs/design/design-system.md`」；若**临时选了锚点**，注明并附上「建议用 d-decision 固化」的提示。
