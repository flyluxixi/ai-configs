...
8. **Color & typography（配色与字体）**【补充维度】：主色 / 品牌色 / 中性色 / 语义色（成功/警告/危险）；字体风格与层级（标题/正文/标注）。
...
Layout: [navigation pattern + grid + section placement].
...
- **gpt-image-2 特性**：prompt 上限 32000 字符（远超 DALL-E 2 的 1000）；文字渲染易错（界面文案 / 海报标语尽量短、用常见词，并提醒用户文字可能不准）；产出是「看起来像」的概念图，不替代 Figma / 代码 / 真实拍摄。
...
4. **推断项清单**（逐条列出你替用户补的风格 / **装饰档位** / **成熟度档** / IA 三层 / 配色 / 组件 / 构图等，可否决；走参考图驱动时附从参考图提取的风格特征 + 推定档位）。若**继承了项目视觉锚点**，注明「已继承 `docs/visual-language.md`」。
