...
```bash
ls ~/.claude/plugins/cache/openai-codex/codex/ 2>/dev/null
```
...
```bash
node "$(ls ~/.claude/plugins/cache/openai-codex/codex/*/scripts/codex-companion.mjs | sort -V | tail -1)" adversarial-review ""
```
...
仅对「必须修复」和「建议修复」的条目执行修复。修复完成后，重新以相同 Bash 命令（`run_in_background: true`）启动复审，**循环直到 Codex 无新的「必须修复」或「建议修复」条目**。
...
- 找到可用命令 → 执行，输出结果摘要
- 未找到任何测试配置 → 告知用户"未发现测试配置，跳过测试步骤"，继续流程
